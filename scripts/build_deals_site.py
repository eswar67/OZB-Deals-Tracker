...
def deals_from_monitor_run(deals: list[dict], generated_at: datetime | None = None) -> tuple[list[dict], datetime]:
    ...
    for deal in deals:
        ...
        row = {
            ...
            "is_today_delta": bool(deal.get("is_delta_deal") or deal.get("is_new_deal") or deal.get("is_best_seen")),
            "delta_reason": deal.get("delta_reason") or ("new" if deal.get("is_new_deal") else ("saving_improved" if deal.get("is_best_seen") else "")),
  ...
  <div class="deals-column">
    ...
    <section class="subscribe-card" id="subscribe" aria-label="Email alerts subscription">
      ...
      <form class="subscribe-form" id="subscribe-form">
        ...
        <div class="subscribe-actions" id="subscribe-actions" hidden>
          <a class="subscribe-action subscribe-action-primary" id="subscribe-gmail" target="_blank" rel="noopener">Open in Gmail</a>
          <a class="subscribe-action" id="subscribe-mailapp">Open mail app</a>
          <button class="subscribe-action" id="subscribe-copy" type="button">Copy request</button>
        </div>
        <p class="subscribe-note" id="subscribe-note">Prepares a pre-filled request — send it via Gmail, your mail app, or copy &amp; paste.</p>
      </form>
    </section>
  </div>
...
SITE_SCRIPT = r"""<script>
  ...
  (function subscribeSetup() {
    // mailto: alone silently fails when the browser has no default mail app,
    // so submitting reveals three working paths: Gmail web compose (works in
    // any browser), the OS mail app, and copy-to-clipboard.
    ...
    const actions = document.querySelector('#subscribe-actions');
    const gmailLink = document.querySelector('#subscribe-gmail');
    const mailLink = document.querySelector('#subscribe-mailapp');
    const copyBtn = document.querySelector('#subscribe-copy');
    const TO = 'eswar67@gmail.com';
    const SUBJECT = 'Subscribe to OZB Deal Radar alerts';
    ...
    form.addEventListener('submit', (event) => {
      ...
      mailLink.href = 'mailto:' + TO + '?subject=' + su + '&body=' + body;
    ...
    copyBtn.addEventListener('click', async () => {
      try {
        await navigator.clipboard.writeText(requestText || buildRequest());
        note.textContent = 'Request copied — paste it into an email to ' + TO + '.';
      } catch (e) {
        note.textContent = 'Copy blocked by the browser — email ' + TO + ' with your address instead.';
      }
    });
